...
function runADASEngine(make, model, year, repairs) {
  ...
  if (systems.length === 0) {
    systems.push('No ADAS calibration flagged for the reported repairs on this vehicle');
  }

  return {
    adasSystems: systems.join('\n'),
    rationale: rationale.join('\n'),
    liabilityWarning: liability,
    makeSpecificNotes: notes.join('\n'),
    preScanRequired: preScan,
    postScanRequired: postScan,
    approvedScanTool: tool,
  };
}
...
function requireSubscription(req, res, next) {
  ...
  if (!shop || shop.subscription_status !== 'active') return res.redirect('/billing/inactive');
  next();
}
...
app.get('/register', (req, res) => {
  ...
  res.send(layout('Register', `
    <div style="max-width:480px;margin:3rem auto;background:#fff;padding:2rem;border-radius:8px;border:1px solid #e0e0e0">
      ...
      <p style="color:#666;margin-bottom:1.5rem;font-size:.9rem">CollisionIQ — $25/month. Cancel anytime.</p>
      ${cancelled ? '<div style="background:#fef2f2;border:1px solid #fca5a5;color:#b91c1c;padding:.6rem;border-radius:4px;margin-bottom:1rem">Payment cancelled. Please try again.</div>' : ''}
      <form method="POST" action="/register">
        ...
        <button type="submit" style="width:100%;padding:.65rem;background:#1a1a2e;color:#fff;border:none;border-radius:4px;font-size:1rem;font-weight:600;cursor:pointer">Continue to Payment →</button>
      </form>
...
app.get('/register/success', async (req, res) => {
  ...
  try {
    ...
    if (checkoutSession.payment_status !== 'paid') return res.redirect('/register?cancelled=1');

    const sub = checkoutSession.subscription;
    const shopResult = db.prepare(`INSERT INTO shops (name, stripe_customer_id, stripe_subscription_id, subscription_status, subscription_current_period_end, created_at) VALUES (?,?,?,'active',?,datetime('now'))`)
      .run(pending.shop_name, pending.stripe_customer_id, sub.id, sub.current_period_end);
